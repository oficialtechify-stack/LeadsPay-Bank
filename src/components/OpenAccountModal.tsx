...
export const OpenAccountModal: React.FC<OpenAccountModalProps> = ({
  ...
  const [showFieldsDirectly, setShowFieldsDirectly] = useState(false);
  ...
  useEffect(() => {
    ...
    if (auth.currentUser?.email) {
      ...
      if (!selfiePhoto && auth.currentUser.photoURL) {
        setSelfiePhoto(auth.currentUser.photoURL);
      }
    }
    ...
    if (draft) {
      ...
      if (draft.fullName || draft.cpf || draft.email) {
        setShowFieldsDirectly(true);
      }
  ...
  const handleGoogleSignInForRegistration = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        const existingUserApp = (user.uid ? getLocalUserApplication(user.uid) : null) || 
                                (user.email ? findApplicationByEmailOrCpf(user.email) : null);
        if (existingUserApp) {
          setExistingApp(existingUserApp);
          setIsGoogleLoading(false);
          if (existingUserApp.status === 'approved') {
            onApplicationSubmitted?.(existingUserApp);
          }
          return;
        }

        // 2. Check if approved profile exists
        if (user.email) {
          const existingProfile = findUserProfileByEmailOrCpf(user.email);
        ...
        // 3. User authenticated via Google: pre-fill fields & show the form!
        if (user.displayName) {
          setFullName(user.displayName);
          setPreferredName(user.displayName.split(' ')[0]);
        }
        if (user.email) {
          setEmail(user.email);
        }
        if (user.photoURL && !selfiePhoto) {
          setSelfiePhoto(user.photoURL);
        }
        setIsGoogleConnected(true);
        setShowFieldsDirectly(true);
      ...
      if (authErr?.code !== 'auth/popup-closed-by-user') {
        setErrorMessage('Não foi possível conectar com o Google. Você pode preencher os dados manualmente.');
        setShowFieldsDirectly(true);
      }
    }
  };
            ...
            <div className="flex items-center gap-2.5 mb-4 shrink-0">
              ...
              <div className="flex-1 pr-6">
                <h3 className="text-base font-display font-bold text-white flex items-center gap-2 flex-wrap">
                  <span>Criar Conta LeadsPay</span>
            ...
            <div className="flex-1 overflow-y-auto pr-1 space-y-4 text-left">
              ...
              {step === 1 && (
                <div className="space-y-3.5">
                  {!showFieldsDirectly && !isGoogleConnected ? (
                    <motion.div
                      initial={{ opacity: 0, y: 8 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="p-5 rounded-2xl bg-gradient-to-b from-white/[0.08] to-white/[0.02] border border-white/15 text-center space-y-4 my-2 shadow-xl"
                    >
                      <div className="w-14 h-14 rounded-2xl bg-white/10 border border-white/20 flex items-center justify-center mx-auto shadow-md">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-300 mt-1 max-w-xs mx-auto leading-relaxed">
                          Aperte em <strong className="text-[#00e676]">Logar com o Google</strong> para conectar seus dados e liberar o preenchimento da sua conta.
                        </p>
                      </div>

                      {/* Botão Logar com o Google */}
                      <button
                        type="button"
                        onClick={handleGoogleSignInForRegistration}
                        disabled={isGoogleLoading}
                        className="w-full py-3.5 px-4 rounded-xl bg-white hover:bg-slate-100 active:scale-[0.98] text-slate-900 font-bold text-sm flex items-center justify-center gap-2.5 transition-all shadow-lg cursor-pointer"
                      >
                        {isGoogleLoading ? (
                          <>
                      ...
                      <div className="pt-2">
                        <button
                          type="button"
                          onClick={() => setShowFieldsDirectly(true)}
                          className="text-xs text-slate-400 hover:text-white underline cursor-pointer"
                        >
                          Ou preencher formulário manualmente sem o Google
                        </button>
                      </div>
                    </motion.div>
                  ) : (
                    ...
                    >
                      ...
                      {isGoogleConnected ? (
                        <div className="p-3 rounded-2xl bg-emerald-500/15 border border-emerald-500/35 flex items-center justify-between gap-2.5">
                          <div className="flex items-center gap-2.5 overflow-hidden">
                            {auth.currentUser?.photoURL ? (
                              <img
                                src={auth.currentUser.photoURL}
                                alt="Avatar Google"
                                className="w-8 h-8 rounded-full border border-emerald-400 object-cover shrink-0"
                              />
                            ) : (
                              <div className="w-8 h-8 rounded-full bg-emerald-500/30 flex items-center justify-center text-[#00e676] shrink-0">
                          ...
                          <span className="text-[10px] bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 px-2.5 py-0.5 rounded-full font-medium shrink-0">
                            ✓ Verificado
                          </span>
                        </div>
                      ) : (
                        <div className="p-2.5 rounded-xl bg-white/[0.04] border border-white/10 flex items-center justify-between gap-2">
                          ...
                          <button
                            ...
                            onClick={handleGoogleSignInForRegistration}
                            disabled={isGoogleLoading}
                            className="text-xs text-[#00e676] hover:underline font-semibold flex items-center gap-1 cursor-pointer"
                          >
                      ...
                      <div className="p-2.5 rounded-xl bg-white/[0.02] border border-white/5 text-[11px] text-slate-300">
                        Preencha os campos abaixo para concluir a abertura da sua conta LeadsPay:
                      </div>
            ...
            <div className="pt-4 border-t border-white/10 mt-3 flex items-center justify-between gap-3 shrink-0">
              ...
              {step < 4 ? (
                <button
                  ...
                  onClick={() => {
                    if (step === 1) {
                      if (!showFieldsDirectly && !isGoogleConnected) {
                        setShowFieldsDirectly(true);
                        return;
                      }
                ...
                >
                  <span>
                    {step === 1 && !showFieldsDirectly && !isGoogleConnected ? 'Preencher Dados' : 'Próxima Etapa'}
                  </span>
